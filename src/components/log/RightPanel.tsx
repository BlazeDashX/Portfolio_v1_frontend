...
export default function RightPanel({
  ...
  // Phase 2: pattern-aware suggestions
  const { data: patterns = [] } = useQuery({
    queryKey: ['patterns', dateStr, currentHour],
    queryFn: () =>
      api.get(`/schedule-templates/patterns?date=${dateStr}&hour=${currentHour}`).then(r => r.data),
    staleTime: 5 * 60_000,
  });
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto" style={{ scrollbarWidth: 'none', gap: '20px' }}>
      ...
      {/* Phase 2: Pattern-aware — "Usually at this time" */}
      {(patterns as any[]).length > 0 && (
        <div>
          <Section label="Usually at this time" sub="Based on your history" />
          <div className="space-y-1.5">
            {(patterns as any[]).map((t: any, i: number) => (
              <Chip key={i} task={t} onSelect={onSelect} badge={`${t.frequency}×`} />
            ))}
          </div>
        </div>
      )}
      ...
      {!hasRegulars && (patterns as any[]).length === 0 && (
        <div>
